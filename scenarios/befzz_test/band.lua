...
local function create_band_gui(event)
  ...
  if player.gui.top.band_toggle_btn == nil then
    local button = player.gui.top.add { name = "band_toggle_btn", type = "sprite-button", caption = "Band", style = "dialog_button_style" }
...
expand_band_gui = function(player, dev_icons, dev_addfakes, right_click)
  ...
  if (frame) then
    ...
    if player.tag ~= "" then
      player.gui.top.band_toggle_btn.tooltip = "Band: "..player.tag.."\n Right Click to show offline players with tags."
  ...
  local frame = player.gui.left.add { type = "frame", direction = "vertical", name = "band_panel", caption = "What are you doing right now:"}
...
local function print_role_change(name, role)
  ...
  if role then
    if roles[role].verbs and math.random() > 0.7 then
      ...
      str = ("[%band] band has `" .. get_random_from_table(roles[role].verbs) .. "` with %name.")
    end
    str = str:gsub('%%band', role)
  elseif local_role then
    str = "%name is not in a band anymore"
    if math.random() > .9 then
      str = str .. " (["..local_role.."] band will miss you)."
    end
  end
...
local function update_player_role(player, role)
  ...
  for _,cplayer in pairs( game.connected_players ) do
    if cplayer.gui.left.band_panel then
      ...
      if role then
        subgui_add_player_label( troles.children[roles[role].index*2], cplayer.name, cplayer.color)
        subgui_update_role_counter( troles.children[roles[role].index*2 - 1].role_cap_line, 1)
      end
  ...
  if role then
    troles_local.children[roles[role].index*2 - 1].children[2].children[1].style.font_color = {r=.7,g=1,b=.7}
  end
  
  print_role_change(player.name, role)
...
local function on_gui_click(event)
  ...
  if (name == "band_clear") then
    ...
    player.gui.top.band_toggle_btn.caption = "Band"
    player.gui.top.band_toggle_btn.tooltip = ""
  ...
  if name:find("band_role_") == 1 then
    if not player.admin and event.tick - band_last_change < option_band_change_interval then
      player.print("[Band] Too fast! Please wait " .. math.floor(1+(band_last_change + option_band_change_interval - event.tick)/60).." s.")
      return
    end
